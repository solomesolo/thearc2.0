// Fixed Mixpanel implementation for static HTML files
(function() {
    'use strict';
    ...
    // Load Mixpanel script with proper initialization
    function loadAndInitializeMixpanel() {
        return new Promise((resolve, reject) => {
            // Check if already loaded and initialized
            if (window.mixpanel && window.mixpanel.__SV && window.mixpanelReady) {
                resolve();
                return;
            }
            ...
            script.onload = function() {
                ...
                // Wait for mixpanel object to be available
                const checkMixpanel = () => {
                    if (window.mixpanel && window.mixpanel.init) {
                        console.log('✅ Mixpanel object available, initializing...');
                        initializeMixpanel();
                        resolve();
                    } else {
                        console.log('⏳ Waiting for Mixpanel object...');
                        setTimeout(checkMixpanel, 100);
                    }
                };
                
                // Start checking after a short delay
                setTimeout(checkMixpanel, 200);
            };
    ...
    async function init() {
        try {
            await loadAndInitializeMixpanel();
